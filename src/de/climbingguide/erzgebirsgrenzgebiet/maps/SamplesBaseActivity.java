...
//				}
	...
	public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
		if (ClimbingGuideApplication.SETTING_SCALE.equals(key)) {
			this.mapView.getModel().displayModel.setUserScaleFactor(DisplayModel.getDefaultUserScaleFactor());
			Log.d(ClimbingGuideApplication.TAG, "Tilesize now " + this.mapView.getModel().displayModel.getTileSize());
			AndroidUtil.restartActivity(this);
		}
		if (ClimbingGuideApplication.SETTING_TILECACHE_PERSISTENCE.equals(key)) {
			if (!preferences.getBoolean(ClimbingGuideApplication.SETTING_TILECACHE_PERSISTENCE, false)) {
				Log.d(ClimbingGuideApplication.TAG, "Purging tile caches");
		...
		if (ClimbingGuideApplication.SETTING_TEXTWIDTH.equals(key)) {
			AndroidUtil.restartActivity(this);
		}
		if (ClimbingGuideApplication.SETTING_TILECACHE_QUEUESIZE.equals(key) || ClimbingGuideApplication.SETTING_TILECACHE_THREADING.equals(key)) {
			AndroidUtil.restartActivity(this);
		}
		if (SETTING_SCALEBAR.equals(key)) {
			setMapScaleBar();
		}
		if (ClimbingGuideApplication.SETTING_DEBUG_TIMING.equals(key)) {
			MapWorker.DEBUG_TIMING = preferences.getBoolean(ClimbingGuideApplication.SETTING_DEBUG_TIMING, false);
		}
		if (ClimbingGuideApplication.SETTING_WAYFILTERING_DISTANCE.equals(key) ||
				ClimbingGuideApplication.SETTING_WAYFILTERING.equals(key)) {
			MapFile.wayFilterEnabled = preferences.getBoolean(ClimbingGuideApplication.SETTING_WAYFILTERING, true);
			if (MapFile.wayFilterEnabled) {
				MapFile.wayFilterDistance = Integer.parseInt(preferences.getString(ClimbingGuideApplication.SETTING_WAYFILTERING_DISTANCE, "20"));
			}
		}
	}
	...
	protected void setMaxTextWidthFactor() {
		mapView.getModel().displayModel.setMaxTextWidthFactor(Float.valueOf(sharedPreferences.getString(ClimbingGuideApplication.SETTING_TEXTWIDTH, "0.7")));
	}
